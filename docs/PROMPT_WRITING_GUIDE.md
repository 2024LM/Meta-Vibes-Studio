...
Write prompts as clear instructions rather than vague ideas. Include only the details needed for the result: subject, action, environment, appearance, continuity, and exclusions. Camera, lighting, or style instructions are optional and should be added only when the user needs them.
...
Style: [optional visual style, if required].
...
Score each category from 0 to 2 before generation. A sufficiently clear prompt should score at least 10/14. Camera and lighting details are optional when they do not affect the requested result.
...
| --- | --- | --- |
| “Make it look good.” | The intended result is undefined. | Describe the required subject, action, appearance, and environment. |
